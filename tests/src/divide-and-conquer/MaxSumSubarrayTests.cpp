/**
 * @file MaxSumSubarrayTests.cpp
 * @author Alan Abraham P Kochumon
 * @date Created on: February 24, 2026
 *
 * @brief Max Sum Subarray Tests.
 *
...
{
    os << "AlgorithmTestsParams( " << "Algorithm: " << param.algo << ", data: { sum: " << param.data.sum
       << ", start: " << param.data.start << ", end: " << param.data.end << " })\n";
    return os;
}
...
TEST_P(MaxSumSubarrayIntegralTestFixture, ReturnsCorrectSubarrayAndSum) { runTests(GetParam()); }

TEST_P(MaxSumSubarrayFloatingPointTestFixture, ReturnsCorrectSubarrayAndSum) { runTests(GetParam()); }



INSTANTIATE_TEST_CASE_P(
    MaxSumSubarrayIntegralTests, MaxSumSubarrayIntegralTestFixture,
    ::testing::Values(
        // Brute Force
        ...
        AlgorithmTestParam<int>(BRUTE_FORCE,
                                { { INT32_MAX, INT32_MAX }, 0, 1, 2 * static_cast<WideType<int>>(INT32_MAX) }),
        ...
        AlgorithmTestParam<int>(DIVIDE_AND_CONQUER,
                                { { INT32_MAX, INT32_MAX }, 0, 1, 2 * static_cast<WideType<int>>(INT32_MAX) }),
        ...
        AlgorithmTestParam<int>(HYBRID, { { 0 }, 0, 0, 0 }), AlgorithmTestParam<int>(HYBRID, { { 7 }, 0, 0, 7 }),
        AlgorithmTestParam<int>(
            HYBRID, { { 13, -3, -25, 20, -3, -16, -23, 18, 20, -7, 12, -5, -22, 15, -4, 7 }, 7, 10, 43, 5 }),
        AlgorithmTestParam<int>(HYBRID, { { INT32_MAX, INT32_MAX }, 0, 1, 2 * static_cast<WideType<int>>(INT32_MAX) }),

        // Kadane's
        AlgorithmTestParam<int>(KADANES, { { -2, 1, -3, 4, -1, 2, 1, -5, 4 }, 3, 6, 6, 3 }),
        AlgorithmTestParam<int>(KADANES, { { -5, -8, -1, -2, -4 }, 2, 2, -1 }),
        AlgorithmTestParam<int>(KADANES, { { 1, 2, 3, 4, 5 }, 0, 4, 15 }),
        AlgorithmTestParam<int>(KADANES, { { 0 }, 0, 0, 0 }), AlgorithmTestParam<int>(KADANES, { { 7 }, 0, 0, 7 }),
        AlgorithmTestParam<int>(
            KADANES, { { 13, -3, -25, 20, -3, -16, -23, 18, 20, -7, 12, -5, -22, 15, -4, 7 }, 7, 10, 43, 5 }),
        AlgorithmTestParam<int>(KADANES,
                                { { INT32_MAX, INT32_MAX }, 0, 1, 2 * static_cast<WideType<int>>(INT32_MAX) })));

INSTANTIATE_TEST_CASE_P(
    MaxSumSubarrayFloatingPointTests, MaxSumSubarrayFloatingPointTestFixture,
    ::testing::Values(
        // Brute Force
        AlgorithmTestParam<float>(BRUTE_FORCE,
                                  { { -2.0f, 1.0f, -3.0f, 4.0f, -1.0f, 2.0f, 1.0f, -5.0f, 4.0f }, 3, 6, 6.0f }),
        ...
        AlgorithmTestParam<float>(BRUTE_FORCE,
                                  { { FLT_MAX, FLT_MAX }, 0, 1, 2 * static_cast<WideType<float>>(FLT_MAX) }),

        // Divide and Conquer
        AlgorithmTestParam<float>(DIVIDE_AND_CONQUER,
                                  { { -2.0f, 1.0f, -3.0f, 4.0f, -1.0f, 2.0f, 1.0f, -5.0f, 4.0f }, 3, 6, 6.0f }),
        ...
        // Kadanes
        AlgorithmTestParam<float>(KADANES,
                                  { { -2.0f, 1.0f, -3.0f, 4.0f, -1.0f, 2.0f, 1.0f, -5.0f, 4.0f }, 3, 6, 6.0f }),
        AlgorithmTestParam<float>(KADANES, { { -5.0f, -8.0f, -1.0f, -2.0f, -4.0f }, 2, 2, -1.0f }),
        AlgorithmTestParam<float>(KADANES, { { 1.0f, 2.0f, 3.0f, 4.0f, 5.0f }, 0, 4, 15.0f }),
        AlgorithmTestParam<float>(KADANES, { { 7.0f }, 0, 0, 7.0f }),
        AlgorithmTestParam<float>(KADANES, { { 0.0f }, 0, 0, 0.0f }),
        AlgorithmTestParam<float>(KADANES, { { FLT_MAX, FLT_MAX }, 0, 1, 2 * static_cast<WideType<float>>(FLT_MAX) })));