...
INSTANTIATE_TEST_CASE_P(
	...
	::testing::Values(
		...
		AlgorithmTestParam<int>(KADANES, { {0}, 0, 0, 0 }),
		AlgorithmTestParam<int>(KADANES, { {7}, 0, 0, 7 }),
		AlgorithmTestParam<int>(KADANES, { {13, -3, -25, 20, -3, -16, -23, 18, 20, -7, 12, -5, -22, 15, -4, 7}, 7, 10, 43, 5 }),
		AlgorithmTestParam<int>(KADANES, { {INT32_MAX, INT32_MAX}, 0, 1, 2 * static_cast<WideType<int>>(INT32_MAX) })
	)
);
...
INSTANTIATE_TEST_CASE_P(
	...
	::testing::Values(
		...
		// Kadanes
		AlgorithmTestParam<float>(KADANES, { {-2.0f, 1.0f, -3.0f, 4.0f, -1.0f, 2.0f, 1.0f, -5.0f, 4.0f}, 3, 6, 6.0f }),
