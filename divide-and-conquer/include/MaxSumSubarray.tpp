#pragma once
/**
 * @file MaxSumSubarray.tpp
 * @author Alan Abraham P Kochumon
 * @date Created on: February 24, 2026
 *
 * @brief MaxSumSubarray implementation.
 *
...
namespace algorithms
{
    ...
    template <std::ranges::contiguous_range Range>
        requires Arithmetic<std::ranges::range_value_t<Range>>
    static std::tuple<std::size_t, std::size_t, WideType<std::ranges::range_value_t<Range>>> crossMaxSumSubarray(
        const Range& elements, std::size_t start, std::size_t middle, std::size_t end)
    {
        using R = WideType<std::ranges::range_value_t<Range>>;
        R leftSum = std::numeric_limits<R>::lowest(),
          rightSum = std::numeric_limits<R>::lowest(); // Initialize to the lowest value of the given data type.
    ...
    template <std::ranges::contiguous_range Range>
        requires Arithmetic<std::ranges::range_value_t<Range>>
    std::tuple<std::size_t, std::size_t, WideType<std::ranges::range_value_t<Range>>> divideAndConquer(
        const Range& elements, std::size_t start, std::size_t end)
    {
        if (start == end)
    ...
    template <std::ranges::contiguous_range Range>
        requires Arithmetic<std::ranges::range_value_t<Range>>
    static std::tuple<std::size_t, std::size_t, WideType<std::ranges::range_value_t<Range>>> hybridSort(
        const Range& elements, std::size_t start, std::size_t end, std::size_t limit)
    {
        if (end - start < limit)
    ...
    template <std::ranges::contiguous_range Range>
        requires Arithmetic<std::ranges::range_value_t<Range>>
    static std::tuple<std::size_t, std::size_t, WideType<std::ranges::range_value_t<Range>>> kadanes(
        const Range& elements, std::size_t size)
    {
        using R = WideType<std::ranges::range_value_t<Range>>;
    ...
    std::tuple<std::size_t, std::size_t, WideType<std::ranges::range_value_t<Range>>> maxSumSubarray(
        const Range& elements, std::size_t size, AlgorithmType algorithm, std::size_t limit)
    {
